 ...
 * Colors: Anthrazit #0D1421 + Cyan #00D4FF + Bernstein #F59E0B
 * Fonts: Syne (headings) + Source Sans 3 (body)
 */
...
import {
  marketStats, marketGrowthData, useCaseData, pestelData,
  swotData, porterData, competitorData, radarData, strategicRecommendations,
...
// ─── PESTEL Factor Card ──────────────────────────────────────────────────────
function PestelCard({ category, data }: { category: string; data: typeof pestelData.political }) {
  ...
  return (
    <div className="energy-card p-6 rounded-xl" style={{ borderColor: `${data.color}33` }}>
      ...
      <p className="text-xs mb-4 leading-relaxed" style={{ color: "#8899BB", fontFamily: "'Source Sans 3', sans-serif" }}>
        {data.summary}
      </p>
      <div className="space-y-2">
        {data.factors.map((factor, idx) => (
          <div key={idx}
            ...
            style={{ background: expanded === idx ? `${data.color}10` : "rgba(255,255,255,0.02)", border: `1px solid ${expanded === idx ? data.color + "40" : "#2A3550"}` }}
...
function SwotQuadrant({
  title, items, color, bgClass, icon,
}: {
  title: string;
  ...
  return (
    <div className={`rounded-xl p-5 border ${bgClass}`}>
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-lg font-bold" style={{ fontFamily: "'Syne', sans-serif", color }}>
          {title}
        </h3>
            ...
            <AnimatePresence>
              {expanded === idx && (
                <motion.p
                  ...
                  style={{ color: "#8899BB", fontFamily: "'Source Sans 3', sans-serif" }}>
                  {item.description}
                </motion.p>
              )}
            </AnimatePresence>
...
// ─── Porter Force Card ───────────────────────────────────────────────────────
function PorterCard({ force }: { force: typeof porterData[0] }) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {force.factors.map((f, i) => (
                <div key={i} className="flex items-center gap-2 text-xs p-2 rounded-lg"
                  ...
                  <span className="ml-auto font-semibold text-xs" style={{ color: f.direction === "enabler" ? "#10B981" : "#EF4444" }}>
                    {f.impact}
                  </span>
                </div>
              ))}
      ...
      <nav className="fixed left-0 top-0 h-full w-64 z-40 hidden lg:flex flex-col"
        ...
        <div className="p-6 border-b" style={{ borderColor: "#1E2D45" }}>
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="text-xs font-bold" style={{ color: "#00D4FF", fontFamily: "'Syne', sans-serif" }}>
                KI-MARKTANALYSE
              </div>
      ...
      <main className="lg:ml-64 pt-14 lg:pt-0">
            ...
            <div className="energy-card p-6 rounded-xl">
              ...
              <div className="flex items-center gap-6">
                <ResponsiveContainer width="50%" height={200}>
                  <PieChart>
                    <Pie data={useCaseData} cx="50%" cy="50%" innerRadius={50} outerRadius={80}
                      paddingAngle={3} dataKey="value">
                      {useCaseData.map((entry, index) => (
                        <Cell key={index} fill={entry.color} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value) => [`${value}%`, ""]} contentStyle={{ background: "#1A2235", border: "1px solid #2A3550", borderRadius: "8px", color: "#E8EDF5", fontSize: "12px" }} />
                  </PieChart>
                </ResponsiveContainer>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <div className="space-y-2 text-sm" style={{ color: "#A0B4CC" }}>
                  {[
                    ["~1.000", "Stadtwerke in Deutschland"],
                    ["~900", "Strom-Netzbetreiber"],
                    ["4", "Übertragungsnetzbetreiber"],
                    ["250+", "Energiedienstleister"],
                  ].map(([num, label]) => (
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    "80% EE-Ziel bis 2030",
                    "600 Mrd. EUR Netzinvestitionen bis 2045",
                    "53 Mio. Smart Meter bis 2032",
                    "15 Mio. E-Autos bis 2030 (Ziel)",
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    "Legacy IT-Infrastruktur (20-30 Jahre alt)",
                    "EU AI Act Compliance-Aufwand",
                    "KRITIS-Cybersecurity-Anforderungen",
                    "Datenschutz (DSGVO) bei Kundendaten",
                    "Investitionsbudgets bei Stadtwerken",
                  ].map((item) => (
                    <div key={item} className="flex items-center gap-2 text-xs" style={{ color: "#A0B4CC" }}>
        ...
        <section id="pestel" ref={(el) => { sectionRefs.current.pestel = el; }}
          ...
          <div className="energy-card p-6 rounded-xl mb-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
                ...
                <div className="space-y-3">
                  {Object.entries(pestelData).map(([key, data]) => (
                    <div key={key} className="flex items-center gap-3">
                      <span className="text-base w-6">{data.icon}</span>
                      <span className="text-xs w-24" style={{ color: "#A0B4CC" }}>{data.label}</span>
                      ...
                      <span className="text-xs font-mono" style={{ color: data.color }}>{data.rating}/5</span>
                    </div>
                  ))}
                </div>
              </div>
              <ResponsiveContainer width="100%" height={250}>
                <RadarChart data={[
                  { subject: "Politisch", A: pestelData.political.rating * 20 },
                  { subject: "Wirtschaftlich", A: pestelData.economic.rating * 20 },
                  { subject: "Sozial", A: pestelData.social.rating * 20 },
                  { subject: "Technologisch", A: pestelData.technological.rating * 20 },
                  { subject: "Ökologisch", A: pestelData.environmental.rating * 20 },
                  { subject: "Rechtlich", A: pestelData.legal.rating * 20 },
                ]}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {Object.entries(pestelData).map(([key, data]) => (
              <PestelCard key={key} category={key.toUpperCase()} data={data} />
            ))}
          </div>
        </section>
        ...
        <section id="swot" ref={(el) => { sectionRefs.current.swot = el; }}
          ...
          <div className="rounded-xl overflow-hidden mb-8 relative h-40">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="grid grid-cols-2 gap-4 text-center">
                {[
                  { label: "Stärken", count: swotData.strengths.length, color: "#10B981" },
                  { label: "Chancen", count: swotData.opportunities.length, color: "#00D4FF" },
                  { label: "Schwächen", count: swotData.weaknesses.length, color: "#F59E0B" },
                  { label: "Risiken", count: swotData.threats.length, color: "#EF4444" },
                ].map((item) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <SwotQuadrant title="Stärken (Strengths)" items={swotData.strengths} color="#10B981" bgClass="swot-strengths" icon="💪" />
            <SwotQuadrant title="Chancen (Opportunities)" items={swotData.opportunities} color="#00D4FF" bgClass="swot-opportunities" icon="🚀" />
            <SwotQuadrant title="Schwächen (Weaknesses)" items={swotData.weaknesses} color="#F59E0B" bgClass="swot-weaknesses" icon="⚠️" />
            <SwotQuadrant title="Risiken (Threats)" items={swotData.threats} color="#EF4444" bgClass="swot-threats" icon="🛡️" />
          </div>
        ...
        <section id="porter" ref={(el) => { sectionRefs.current.porter = el; }}
          ...
          <div className="energy-card p-6 rounded-xl mb-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={porterData.map(f => ({ name: f.icon, score: f.score, color: f.color }))}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1E2D45" />
        ...
        <section id="competitors" ref={(el) => { sectionRefs.current.competitors = el; }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              {[
                {
                  category: "Internationale Großberatungen",
                  color: "#A855F7",
                  players: ["Accenture", "McKinsey & Company", "BCG X", "Deloitte", "PwC", "KPMG"],
                  description: "Dominieren durch globale Ressourcen, KI-Investitionen in Milliardenhöhe und breite Industriepräsenz. Accenture ist mit über 1.000 KI-Projekten im Energiesektor Marktführer.",
                },
                {
                  category: "IT-Dienstleister & Technologieanbieter",
                  color: "#0EA5E9",
                  players: ["IBM", "Capgemini Invent", "SAP", "Siemens Energy", "T-Systems", "Atos/Eviden"],
                  description: "Stärke durch proprietäre Technologieplattformen und tiefe System-Integration. SAP IS-U und Siemens MindSphere sind Marktstandards in der Energiewirtschaft.",
                },
                {
                  category: "Spezialisierte Energie-KI-Startups",
                  color: "#00D4FF",
                  players: ["Envelio (Netzplanung)", "Ampeers Energy", "gridX", "Octopus Energy", "Flexidao", "Enpal"],
                  description: "Agile Nischenanbieter mit tiefer Domänenexpertise. Wachsen schnell durch fokussierte Lösungen, fehlen aber oft Skalierungskapazität und regulatorische Breite.",
                },
              ].map((cat, idx) => (
                <div key={idx} className="energy-card p-5 rounded-xl" style={{ borderColor: `${cat.color}30` }}>
                  <div className="font-bold text-sm mb-2" style={{ fontFamily: "'Syne', sans-serif", color: cat.color }}>
                    {cat.category}
                  </div>
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {cat.players.map((p) => (
                      <span key={p} className="text-xs px-2 py-0.5 rounded"
                        style={{ background: `${cat.color}15`, color: "#C8D4E8", border: `1px solid ${cat.color}25` }}>
                        {p}
                      </span>
                    ))}
                  </div>
        ...
        <section id="strategy" ref={(el) => { sectionRefs.current.strategy = el; }}
              ...
              <motion.div key={idx}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="font-bold text-sm mb-2" style={{ fontFamily: "'Syne', sans-serif", color: "#E8EDF5" }}>
                      {rec.title}
                    </h3>
          ...
          {/* Market Outlook */}
          <div className="energy-card p-6 rounded-xl mb-8">
            <h3 className="text-lg font-bold mb-5" style={{ fontFamily: "'Syne', sans-serif", color: "#E8EDF5" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                {
                  year: "2025–2026",
                  title: "Konsolidierungsphase",
                  color: "#00D4FF",
                  points: [
                    "EU AI Act Compliance wird Pflicht",
                    "Generative KI für Backoffice-Automatisierung",
                    "Erste Stadtwerke-KI-Kooperationen",
                    "Marktbereinigung bei Startups",
                  ],
                },
                {
                  year: "2027–2028",
                  ...
                  color: "#10B981",
                  points: [
                    "Smart Meter Rollout treibt Datenprojekte",
                    "KI-Netzplanung wird Industriestandard",
                    "Digitale Zwillinge für Netze verbreitet",
                    "E-Mobilitäts-KI-Lösungen skalieren",
                  ],
                },
                {
                  year: "2029–2030",
                  title: "Transformationsphase",
                  color: "#F59E0B",
                  points: [
                    "Autonome KI-Agenten im Netzbetrieb",
                    "Wasserstoff-KI-Infrastruktur entsteht",
                    "Marktgröße >800 Mio. USD in DE",
                    "Neue Geschäftsmodelle durch KI-Plattformen",
                  ],
                },
              ].map((phase, idx) => (
          ...
          <div className="rounded-xl p-6 relative overflow-hidden"
            ...
            <h3 className="text-xl font-bold mb-3" style={{ fontFamily: "'Syne', sans-serif", color: "#E8EDF5" }}>
              Fazit: Ein Markt im Aufbruch
            </h3>
            <p className="text-sm leading-relaxed max-w-3xl" style={{ color: "#A0B4CC" }}>
              Der KI-Beratungs- und Implementierungsmarkt für die deutsche Energiewirtschaft befindet sich an einem strategischen Wendepunkt. Die Kombination aus Energiewende-Druck, regulatorischem Wandel (EU AI Act) und technologischer Reife schafft ein Marktfenster, das in den nächsten 3–5 Jahren entschieden wird. Anbieter, die tiefe Branchenexpertise mit KI-Kompetenz und regulatorischem Know-how verbinden, werden überproportional von einem Markt profitieren, der bis 2033 auf über 1,1 Mrd. USD anwachsen soll. Die größte Differenzierungsmöglichkeit liegt dabei nicht in der Technologie selbst, sondern in der Fähigkeit, komplexe Energiesystemzusammenhänge in skalierbare, compliance-konforme KI-Lösungen zu übersetzen.
            </p>
          </div>
        </section>
        ...
        <footer className="px-6 md:px-12 py-8 mt-4" style={{ borderTop: "1px solid #1E2D45" }}>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <div className="text-xs" style={{ color: "#445566" }}>
                Marktanalyse 2025 · PESTEL · SWOT · Porter's Five Forces
              </div>
            </div>
            <div className="text-xs" style={{ color: "#445566" }}>
